...
define(["lib/helper"], function (Helper) {
  ...
  function prettyPeer(d) {
    if (d === null)
      ...
      return "prisijungęs (" + prettyUptime(d.established) + ")"
  }
  ...
  function prettyNVRAM(usage) {
    return Helper.formatNumber(usage * 100, 3) + "% užimta"
  }
  ...
  return function (stream) {
    ...
    children.push(mkRow(table, "Uptime", stream.map(".uptime").map(prettyUptime)))
    children.push(mkRow(table, "Apkrova", stream.map(".loadavg").map(prettyLoad)))
    ...
    children.push(mkRow(table, "Vartotojai", stream.map(".clients.total")))

    el.appendChild(table)

    var h = document.createElement("h3")
    h.textContent = "Srautas"
    el.appendChild(h)

    table = document.createElement("table")

    mkTrafficRow(table, children, "Išsiųstas", stream, ".traffic.tx")
    mkTrafficRow(table, children, "Gautas", stream, ".traffic.rx")
    mkTrafficRow(table, children, "Tranzitas", stream, ".traffic.forward")
    ...
    return { title: document.createTextNode("Statistika")
