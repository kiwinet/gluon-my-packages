...
const unsigned int KIWI_INVALID_CHANNEL = 0;
const unsigned int KIWI_INVALID_TXPOWER = 0;
...
static inline unsigned char parse_channel(const char *s) {
        return parse_option(s, KIWI_INVALID_CHANNEL);
}

static inline unsigned char parse_txpower(const char *s) {
        return parse_option(s, KIWI_INVALID_TXPOWER);
}

struct kiwi_wifi_info *kiwi_get_wifi_info(void) {
	struct uci_package *p = NULL;
	struct uci_context *ctx = NULL;
	struct kiwi_wifi_info *ret = NULL;
